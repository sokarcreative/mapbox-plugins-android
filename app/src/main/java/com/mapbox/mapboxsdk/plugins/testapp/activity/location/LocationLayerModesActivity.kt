...
class LocationLayerModesActivity : AppCompatActivity(), OnMapReadyCallback, LocationEngineListener, OnLocationLayerClickListener, OnCameraTrackingChangedListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        buttonLocationMode.setOnClickListener {
            locationLayerPlugin?.let {
                showModeListDialog()
            }
        }

        buttonLocationTracking.setOnClickListener {
            locationLayerPlugin?.let {
    ...
    override fun onMapReady(mapboxMap: MapboxMap) {
        ...
        locationLayerPlugin!!.cameraMode = cameraMode
    ...
    fun toggleMapStyle() {
        val styleUrl = if (mapboxMap!!.styleUrl!!.contentEquals(Style.DARK)) Style.LIGHT else Style.DARK
    ...
    private fun setRendererMode(@RenderMode.Mode mode: Int) {
        ...
        if (mode == RenderMode.NORMAL) {
            buttonLocationMode.text = "Normal"
        } else if (mode == RenderMode.COMPASS) {
            buttonLocationMode.text = "Compass"
        } else if (mode == RenderMode.GPS) {
            buttonLocationMode.text = "Gps"
        }
    }
    ...
    override fun onCameraTrackingChanged(currentMode: Int) {
        ...
        if (cameraMode == CameraMode.NONE) {
            ...
            buttonLocationTracking.text = "Tracking"
        } else if (cameraMode == CameraMode.TRACKING_COMPASS) {
            buttonLocationTracking.text = "Tracking Compass"
        } else if (cameraMode == CameraMode.TRACKING_GPS) {
            buttonLocationTracking.text = "Tracking GPS"
        } else if (cameraMode == CameraMode.TRACKING_GPS_NORTH) {
            buttonLocationTracking.text = "Tracking GPS North"
        }
    }
